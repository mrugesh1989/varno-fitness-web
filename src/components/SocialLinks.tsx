...
type SocialLinksProps = {
  /** Visual style: solid pill buttons (hero) or subtle icon row (footer). */
      ...
      <div className={`flex items-center gap-3 ${className}`}>
        {networks.map((n) => (
          <a
            ...
            className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/[0.04] px-4 py-2 text-sm font-semibold text-white transition hover:border-brand-accent/50 hover:bg-brand-accent/15 hover:text-white"
          >
            {n.icon}
            <span>{n.name}</span>
          </a>
    ...
    <div className={`flex items-center gap-3 ${className}`}>
      {networks.map((n) => (
        <a
          ...
          rel="noreferrer noopener"
          aria-label={`Follow Varno Fitness on ${n.name}`}
          className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-white/10 bg-white/[0.04] text-stone-300 transition hover:border-brand-accent/50 hover:bg-brand-accent hover:text-white"
        >
          {n.icon}
        </a>
      ))}
